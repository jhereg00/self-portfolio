 ...
 *  Work object
 */
var Interactive = function (fileName) {
...
Interactive.prototype = {
   ...
   *  @param {function (this Interactive object)} Callback function, if needed
   */
  load: function (cb) {
    var _this = this;
    // read the file
    fs.readFile(DATA_PATH + this.fileName, function (err, data) {
      if (err) {
        console.error('Error Loading Interactive. FileName: ' + _this.fileName);
        console.error(err);
        console.log('\n');
        // take self out of lists
        allInteractive.splice(allInteractive.indexOf(_this),1);
        sortLists();
      }
      else {
        // successfully loaded
...
function loadAllFromData () {
  fs.readdir(DATA_PATH, function (err, data) {
    if (err) {
      console.error('Error Loading Interactive. Could Not Read Directory: ' + DATA_PATH);
      console.error(err);
      console.log('\n');
    }
    else {
      for (var file in data) {
        ...
        if (/^[^_].*\.md$/.test(data[file])) {
          // "valid" markdown file
          allInteractive.push(new Interactive(data[file]));
        }
      }
    }
  });
...
fs.watch(DATA_PATH, function (event, fileName) {
  ...
  if (/^[^_].*\.md$/.test(fileName)) {
    if (event === 'rename') {
      for (var i = 0, len = allInteractive.length; i < len; i++) {
        if (allInteractive[i].fileName === fileName) {
          allInteractive.splice(i,1);
          break;
        }
      }
    }
    var existingInteractive;
    for (var i = 0, len = allInteractive.length; i < len; i++) {
      if (allInteractive[i].fileName === fileName) {
        allInteractive[i].load();
        existingInteractive = true;
        break;
      }
    }
    if (!existingInteractive) {
      allInteractive.push(new Interactive(fileName));
    }
  }
});
...
function getBy (prop, val, within, listName) {
  ...
  var ret = [];
  var list = listName && lists[listName] ? lists[listName] : allInteractive;
  if (!within) {
